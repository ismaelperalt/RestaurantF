...
const categoryConfig: Record<string, { label: string; emoji: string; color: string }> = {
  entrada:   { label: "Entradas",    emoji: "🥗", color: "#f59e0b" },
  principal: { label: "Principales", emoji: "🍽️", color: "#ef4444" },
  postre:    { label: "Postres",     emoji: "🍮", color: "#8b5cf6" },
  bebida:    { label: "Bebidas",     emoji: "🥤", color: "#3b82f6" },
};

const categoryOrder = ["entrada", "principal", "postre", "bebida"];
...
export default function OrderCard({ order, onAction, onItemStatusChange, onCardClick }: Props) {
  ...
  const grouped = categoryOrder.reduce((acc, cat) => {
    const items = order.items.filter((item) => item.category === cat);
    ...
    >
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          ...
          {order.delayed && (
            <span style={{
              fontSize: 10, fontWeight: 800, background: "#fee2e2",
              color: "#b91c1c", padding: "2px 8px", borderRadius: 6,
            }}>DEMORADO</span>
          )}
        </div>
                  ...
                  {items.map((item) => {
                    ...
                    return (
                      <div key={realIndex} style={{
                        ...
                        <div>
                          <span style={{ color: item.status === "served" ? "#9ca3af" : "#374151", display: "flex", alignItems: "center" }}>
                            {item.emoji.startsWith("http") ? (
                              <img src={item.emoji} alt={item.name} style={{ width: 24, height: 24, objectFit: "cover", borderRadius: 4, marginRight: 6 }} />
                            ) : (
                              <span style={{ fontSize: 16, marginRight: 6 }}>{item.emoji}</span>
                            )}
                        ...
                        <button
                          onClick={(e) => {
                            e.stopPropagation();
                            item.status !== "served" && onItemStatusChange(order.id, realIndex, statusCycle[item.status]);
                          }}
                          style={{
                            fontSize: 11, fontWeight: 700, padding: "3px 10px", borderRadius: 20,
                            background: icfg.bg, color: icfg.text, border: `1px solid ${icfg.border}`,
                            cursor: item.status === "served" ? "default" : "pointer", whiteSpace: "nowrap",
                          }}
                        >
                          {icfg.label} {item.status !== "served" && "›"}
                        </button>
                      </div>
                    );
                  })}
          ...
          {order.items.map((item, i) => {
            ...
            return (
              <div key={i} style={{
                ...
                <div>
                  <span style={{ color: isServed ? "#9ca3af" : "#374151", display: "flex", alignItems: "center" }}>
                    {item.emoji.startsWith("http") ? (
                      <img src={item.emoji} alt={item.name} style={{ width: 28, height: 28, objectFit: "cover", borderRadius: 4, marginRight: 6 }} />
                    ) : (
                ...
                <button
                  onClick={(e) => {
                    e.stopPropagation();
                    !isServed && onItemStatusChange(order.id, i, statusCycle[item.status]);
                  }}
                  style={{
                    fontSize: 11, fontWeight: 700, padding: "3px 10px", borderRadius: 20,
                    background: cfg.bg, color: cfg.text, border: `1px solid ${cfg.border}`,
                    cursor: isServed ? "default" : "pointer", whiteSpace: "nowrap",
                  }}
                >
                  {cfg.label} {!isServed && "›"}
                </button>
